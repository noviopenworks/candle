...
## openspec/changes/rename-to-candle/proposal.md

- Source: openspec/changes/rename-to-candle/proposal.md
- Lines: 1-26
...
The Go module was published under a placeholder path (`github.com/candle/intel-mcp`) and an `intel-mcp` binary sub-name that do not match the canonical repository, `github.com/noviopenworks/candle`. Aligning the module path and binary/command name with the real repo is required before the module can be imported or installed by its true path.

## What Changes

- **BREAKING** (import path): Go module path `github.com/candle/intel-mcp` → `github.com/noviopenworks/candle`. Every internal import is updated accordingly.
...
## openspec/changes/rename-to-candle/design.md

- Source: openspec/changes/rename-to-candle/design.md
- Lines: 1-40
...
## openspec/changes/rename-to-candle/tasks.md

- Source: openspec/changes/rename-to-candle/tasks.md
- Lines: 1-26
- SHA256: 55bb7c6762ec572a89a308cb05fb365f4cc88e454a505c8ccfbeddcd30ba92ce

```md
# Tasks — rename-to-candle
...
- [ ] 1.1 Update `go.mod` module directive to `github.com/noviopenworks/candle`
- [ ] 1.2 Update all internal `.go` import paths to the new module path (pass 1, before bare-name pass)

## 2. Binary / command name

- [ ] 2.1 `git mv cmd/intel-mcp cmd/candle`
- [ ] 2.2 Update cobra root `Use` to `candle` in `cmd/candle/main.go`
- [ ] 2.3 Update MCP server `Name` to `candle` in `internal/mcp/server.go`
...
## openspec/changes/rename-to-candle/specs/module-identity/spec.md

- Source: openspec/changes/rename-to-candle/specs/module-identity/spec.md
- Lines: 1-35
...
- **THEN** the module directive reads `module github.com/noviopenworks/candle`

#### Scenario: internal imports resolve under the canonical path

- **WHEN** the module is built with `go build ./...`
- **THEN** every internal import path begins with `github.com/noviopenworks/candle/` and the build succeeds

### Requirement: Single canonical binary name

The project SHALL ship under a single binary/command name, `candle`, used consistently for the command entrypoint, the cobra root command, and the MCP server identity.

#### Scenario: command and server identify as candle

- **WHEN** the command directory, cobra root `Use`, and MCP server `Name` are inspected
- **THEN** each is `candle` and the entrypoint lives at `cmd/candle/`

### Requirement: No stale prior name in tracked source

Tracked source, configuration, and documentation (excluding generated `graphify-out/` artifacts and archived openspec changes) SHALL contain no references to the prior `intel-mcp` name or the prior `github.com/candle/intel-mcp` module path.
