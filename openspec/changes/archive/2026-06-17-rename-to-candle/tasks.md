...
- [x] 1.1 Update `go.mod` module directive to `github.com/noviopenworks/candle`
- [x] 1.2 Update all internal `.go` import paths to the new module path (pass 1, before bare-name pass)

## 2. Binary / command name

- [x] 2.1 `git mv cmd/intel-mcp cmd/candle`
- [x] 2.2 Update cobra root `Use` to `candle` in `cmd/candle/main.go`
- [x] 2.3 Update MCP server `Name` to `candle` in `internal/mcp/server.go`
