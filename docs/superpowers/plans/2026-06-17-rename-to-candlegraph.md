...
> (`github.com/vend-ai/intel-mcp` → `github.com/noviopenworks/candlegraph`,
> binary `intel-mcp` → `candlegraph`). Execution = commit the existing verified diff
> and tick the tasks against it. No new code is written.

## Task 1 — Module path

- Update `go.mod` module directive → `github.com/noviopenworks/candlegraph`.
...
- `git mv cmd/intel-mcp cmd/candlegraph`.
- Cobra root `Use: "candlegraph"` in `cmd/candlegraph/main.go`.
- MCP server `Name: "candlegraph"` in `internal/mcp/server.go`.
