...
**Module path:** `github.com/noviopenworks/candle` (local/private; not published — adjust if a real remote is chosen).
...
Run: `go mod init github.com/noviopenworks/candle && go test ./internal/version/`
...
Create `cmd/candle/main.go` (placeholder, fleshed out in Task 9/12):
...
func main() {
	fmt.Println("candle", version.String())
}
```
...
func Serve(ctx context.Context, s *store.Store) error {
	tools := NewTools(s)
	srv := mcpsdk.NewServer("candle", "0.1.0-dev", nil)
...
Replace `cmd/candle/main.go` with a cobra root plus `serve` and `index` commands:
